...
class EscaperTest extends IntegrationTestCase
{
    ...
    public function testBasic(): void
    {
        ...
        assertEquals(self::VALUE, $this->mpdo->select(self::TABLE)->columns(['name' => self::COLUMN])->where('id', 1)->row()['name']);
        assertTrue($this->mpdo->update(self::TABLE)->set([self::COLUMN => 'test'])->where('id', 1)->execute());
        assertEquals('test', $this->mpdo->select(self::TABLE)->columns(['name' => self::COLUMN])->where('id', 1)->row()['name']);
